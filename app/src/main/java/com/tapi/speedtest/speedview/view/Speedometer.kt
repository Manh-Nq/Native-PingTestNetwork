package com.tapi.speedtest.ui.speedparameter.speedview.view
    ...
    var tickNumber: Int
        ...
        set(tickNumber) {
            ...
            for (i in 0 until tickNumber) {
                Log.d("TAG", "tickNumber  : ${tickEach * i} - $i")
...
//        indicator = NoIndicator(context)
    ...
    private fun initAttributeSet(context: Context, attrs: AttributeSet?) {
        ...
        tickPadding =
            a.getDimension(R.styleable.Speedometer_sv_tickPadding, tickPadding.toFloat()).toInt()
    ...
    protected fun drawMarks(canvas: Canvas) {
        ratioPath.reset()
        ratioPath.moveTo(size * .5f, marksPadding + padding)
        ratioPath.lineTo(size * .5f, marksPadding + markHeight + padding)
    ...
    protected fun drawTicks(c: Canvas) {
        ...
        ticks.forEachIndexed { index, t ->
            ...
            tick = if (tick.toString().toInt() == maxSpeed.toInt()) {
                tick
            } else {
                "${(tick.toString().toInt() % 10) * 10}"
            }
            Log.d("TAG", "drawTicks: $tick endDegree ${maxSpeed.toInt()}")
            StaticLayout(
                tick,
                textPaint,
                size,
